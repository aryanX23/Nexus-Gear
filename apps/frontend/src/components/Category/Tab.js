...
const ProductCard = ({ item, onProductClick, onAddToCartClick }) => {
    return (
        <div
            className="flex flex-col bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-md group transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-2"
        >
            <div
            ...
            >
                ...
                <img className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" src={item.imageurl} alt={item.name} />
            ...
            <div className="p-4 flex flex-col flex-grow">
                ...
                <div className="flex items-end justify-between mt-4">
                    <p className="text-xl font-bold text-slate-900">
                        <span className="text-base font-medium text-slate-500 mr-0.5">&#8377;</span>
...
const EmptyState = () => (
    <div className="col-span-full flex flex-col items-center justify-center text-center py-20 bg-white/50 backdrop-blur-sm rounded-xl">
        ...
        <h2 className="text-3xl font-bold text-slate-700 mb-2">No Products Found</h2>
        <p className="max-w-md text-slate-500">It seems this category is empty at the moment. Please check back later or try another category.</p>
    </div>
);
...
const Tabs = () => {
    ...
    useEffect(() => {
        if (isLoading) {
            const fetchProducts = async () => {
                try {
                    const response = await Axios.get(`/api/products/${categories[activeTab].id}`);
    ...
    const handleAddToCart = (e, item) => {
        e.stopPropagation();
        addToCart({ _id: item._id, name: item.name, quantity: 1, price: item.price, imageurl: item.imageurl });
    ...
    return (
        <section id="categories" className="relative py-24 sm:py-28 bg-slate-50 overflow-hidden">
            <div aria-hidden="true" className="absolute top-0 left-0 -translate-x-1/2 w-[1000px] h-[1000px] bg-gradient-to-tr from-purple-300/80 to-cyan-300/80 rounded-full blur-[250px] opacity-40"></div>
            <div aria-hidden="true" className="absolute bottom-0 right-0 translate-x-1/3 w-[1000px] h-[1000px] bg-gradient-to-br from-blue-300/80 to-teal-300/80 rounded-full blur-[150px] opacity-40"></div>

            <div className="relative container mx-auto px-4">
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">
                        Explore Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-blue-600">Arsenal</span>
                    </h2>
                ...
                <div className="flex justify-center mb-12">
                    <div className="relative inline-flex flex-nowrap items-center p-2 bg-white/70 border border-white/80 backdrop-blur-lg rounded-xl overflow-x-auto shadow-sm
