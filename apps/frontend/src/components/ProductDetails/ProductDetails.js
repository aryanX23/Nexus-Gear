...
const StarRating = ({ rating, totalReviews }) => (
  <div className="flex items-center gap-2">
    ...
    {totalReviews > 0 && (
      <span className="text-sm text-slate-500 hover:text-blue-600 cursor-pointer font-medium">{`(${totalReviews} reviews)`}</span>
    )}
  </div>
);
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        const reviewResponse = await Axios.post("/api/products/getReviews", {
          productId: props.productId,
        });
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="flex flex-col">
              ...
              <p className="mt-8 text-4xl font-extrabold text-slate-900">
                <span className="text-2xl font-medium text-slate-500 mr-1">
                  &#8377;
                </span>
                  ...
                  <div className="lg:col-span-8 space-y-8">
                    ...
                    {reviews.length > 0 ? (
                      reviews.map((rev, index) => (
                        <article
                          key={index}
                          className="border-t border-slate-200 pt-8 flex items-start gap-4"
                        >
